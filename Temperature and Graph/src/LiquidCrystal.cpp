...
{
	// implement with RIT
	uint64_t compareValue = us*(Chip_RIT_GetBaseClock(LPC_RITIMER)/1000000);
...
void LiquidCrystal::print(std::string const &s, int line)
{
	char buffer[100];
	strncpy(buffer, s.c_str(), sizeof(s));
	buffer[sizeof(s) - 1] = 0;
	this->print(buffer, line);
}

void LiquidCrystal::print(const char *s, int line)
{
	int i = 0;
	setCursor(0,line);
	while(s[i] != '\0'){
